...
@section('content')
    <div class="container">
        <div class="d-flex justify-content-start justify-content-md-between mb-3">
            <h3 class="text-center"><strong>{{ __('Edit Poundfit Event') }}</strong></h3>
        ...
        <div class="row">
            ...
            <div class="col-12 col-md-4 mx-auto">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('poundfit-events.update', $poundfit_event->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            <div class="form-group mb-3">
                                ...
                                <select id="location_id" name="location_id" class="form-select" required>
                                    ...
                                    @foreach ($locations as $location)
                                        <option @selected($poundfit_event->location_id == $location->id) value="{{ $location->id }}">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="date" id="event_date" name="event_date" class="form-control"
                                        value="{{ old('event_date') ?? $poundfit_event->event_date_only }}" required />
                                    <span class="input-group-text">Jam</span>
                                    <input type="time" id="event_time" name="event_time" class="form-control"
                                        value="{{ old('event_time') ?? $poundfit_event->event_time_only }}" required />
                                </div>
                            </div>
                            <div class="form-group mb-3">
                                ...
                                <input type="tel" id="pic_whatsapp" name="pic_whatsapp" class="form-control"
                                    ...
                                    value="{{ old('pic_whatsapp') ?? $poundfit_event->pic_whatsapp }}" required />
                            </div>
                            <div class="form-group mb-3">
                                <label for="is_published" class="form-label">Status</label>
                                <select id="is_published" name="is_published" class="form-select" required>
                                    <option @selected($poundfit_event->is_published) value="1">Aktif</option>
                                    <option @selected(!$poundfit_event->is_published) value="0">Tidak Aktif</option>
                                </select>
                            </div>
                            <div class="form-group mb-3">
                                ...
                                <input type="number" id="registrant_limit" name="registrant_limit" class="form-control"
                                    ...
                                    value="{{ old('registrant_limit') ?? $poundfit_event->registrant_limit }}"
                                    min="0" required />
                            </div>
